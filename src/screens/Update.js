...
export default function Update({ navigation, route }) {
  ...
  const [image, setImage] = useState(rImage);
  const [imageUploading, setImageUploading] = useState(null);
  const [loading, setLoading] = useState(false);

  const [checkedShift, setCheckedShift] = useState(rShift);

  const [name, setName] = useState(rName);
  const [age, setAge] = useState(rAge);
  const [gender, setGender] = useState(rGender);
  ...
  return (
    <SafeAreaView style={style.body}>
      <Header title={"Update Employee"} backBtn={true} />
      ...
      {loading ? (
        ...
        <Button onPress={updateEmployee} title={"Update"} style={style.btn} />
      )}
    </SafeAreaView>
  );
}
